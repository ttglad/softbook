    ...
    <nav class="navbar-default navbar-static-side" role="navigation">
        ...
        <a href="/index">
            <li class="logo hidden-xs">
                <span class="logo-lg">{{ $softShortName }}</span>
            </li>
        </a>
            ...
            <ul class="nav" id="side-menu">
                ...
                @if(count($menus) > 0)
                    ...
                    @foreach($menus as $firstMenu)
                        <li>
                            <a class="@if(!empty($firstMenu['url']) && $firstMenu['url'] != '#') {{ $firstMenu['target'] }} @endif {{ $firstMenu['class'] }}" href="{{ $firstMenu['url'] ?? '#'  }}" data-refresh="{{ $firstMenu['is_refresh'] == 0 ? 'ture' : 'false' }}">
                                <i class="{{ $firstMenu['icon'] }}"></i>
                                <span class="nav-label" text="{{ $firstMenu['menu_name'] }}">{{ $firstMenu['menu_name'] }}</span>
                                <span class="@if(!empty($firstMenu['url']) || $firstMenu['url'] == '#') fa arrow @endif"></span>
                            </a>
                                    ...
                                    @foreach($firstMenu['children'] as $secondMenu)
                                        <li>
                                            @if(empty($secondMenu['children']))
                                                <a class="@if(empty($secondMenu['target'])) menuItem @else {{ $secondMenu['target'] }} @endif {{ $secondMenu['class'] }}" href="{{ $secondMenu['url'] }}" data-refresh="{{ $secondMenu['is_refresh'] == 0 ? 'ture' : 'false' }}">{{ $secondMenu['menu_name'] }}</a>
                                            @else
                                                        ...
                                                        @foreach($secondMenu['children'] as $thirdMenu)
                                                            <li>
                                                                @if(empty($thirdMenu['children']))
                                                                    <a class="@if(empty($thirdMenu['target'])) menuItem @else {{ $thirdMenu['target'] }} @endif {{ $thirdMenu['class'] }}" href="{{ $thirdMenu['url'] }}" data-refresh="{{ $thirdMenu['is_refresh'] == 0 ? 'ture' : 'false' }}">{{ $thirdMenu['menu_name'] }}</a>
                                                                @else
                                                                    <a href="#">{{ $thirdMenu['menu_name'] }}<span class="fa arrow"></span></a>
                                                                    <ul class="nav nav-third-level">
                                                                            ...
                                                                            @foreach($thirdMenu['children'] as $fourthMenu)
                                                                                <li>
                                                                                    @if(empty($fourthMenu['children']))
                                                                                        <a class="@if(empty($fourthMenu['target'])) menuItem @else {{ $fourthMenu['target'] }} @endif {{ $fourthMenu['class'] }}" href="{{ $fourthMenu['url'] }}" data-refresh="{{ $fourthMenu['is_refresh'] == 0 ? 'ture' : 'false' }}">{{ $fourthMenu['menu_name'] }}</a>
                                                                                    @else
                                                                                        <a href="#">{{ $fourthMenu['menu_name'] }}<span class="fa arrow"></span></a>
                                                                                    @endif
                                                                                </li>
                                                                            @endforeach
                                                                        @endif
