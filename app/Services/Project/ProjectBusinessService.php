...
class ProjectBusinessService extends Service
{
     ...
     * @return mixed
     */
    public function businessQuery($business, $businessColumn, $request)
    {
        // 初始化查询条件
        $businessData = ProjectBusiness::where('menu_id', $business->menu_id);

        // 增加查询条件
        foreach ($businessColumn as $column) {
            if ($column->is_query == 1 && $request->exists($column->dict_value) && !empty(trim($request->post($column->dict_value)))) {
                switch (strtoupper($column->query_type)) {
                    case 'LIKE':
                        $businessData = $businessData->where('value_' . ($column->sort - 1), 'like',
                            '%' . trim($request->post($column->dict_value)) . '%');
                        break;
                    default:
    ...
    public function businessDataFormatSingle($businessData, $columns): array
    {
        ...
        foreach ($columns as $column) {
            if (isset($temp[$column->dict_value])) {
                $return[$column->dict_value] = $temp[$column->dict_value];
            } else {
                $return[$column->dict_value] = '';
            }
        }
        return $return;
    }
