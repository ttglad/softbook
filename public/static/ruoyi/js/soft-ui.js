    ...
    $.extend({
                ...
                $.modal.confirm("确定导出所有" + table.options.modalName + "吗？", function() {
                    ...
                    $.post(table.options.exportUrl, dataParam, function(result) {
                        if (result.code == web_status.SUCCESS) {
                            window.location.href = ctx + "business/download?fileName=" + encodeURI(result.msg) + "&delete=" + true;
            ...
            selectDictLabel: function(datas, value) {
                ...
                $.each(datas, function(index, dict) {
                    if (dict.dict_value == ('' + value)) {
                        var list_class = $.common.equals("default", dict.list_class) || $.common.isEmpty(dict.list_class) ? "" : "badge badge-" + dict.list_class;
                        actions.push($.common.sprintf("<span class='%s'>%s</span>", list_class, dict.dict_label));
                        return false;
                    }
                });
                if (actions.length === 0) {
        ...
        common: {
            ...
            dictToSelect: function(datas, value, name) {
                ...
                $.each(datas, function(index, dict) {
                    actions.push($.common.sprintf("<option value='%s'", dict.dict_value));
                    if (dict.dict_value == ('' + value)) {
                        actions.push(' selected');
                    }
                    actions.push($.common.sprintf(">%s</option>", dict.dict_label));
                });
                actions.push('</select>');
                return actions.join('');
            },
